...
function findMinDelete(node, mustFree) {
    let bestSize = Number.MAX_VALUE;
    if (node.type === 'directory') {
        if (node.size > mustFree) {
            bestSize = node.size;
        }

        for (const child of node.childNodes) {
            if (child.type === 'directory') {
                let bestChildSize = findMinDelete(child, mustFree);
                bestSize = Math.min(bestSize, bestChildSize);
            }
        }
    }
...
async function runner() {
    ...
    const unused = disk - used;
    const mustFree = requiredFree - unused;

    const bestSize = findMinDelete(tree, mustFree);
