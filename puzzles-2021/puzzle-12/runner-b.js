...
function nextPaths(path, edges) {
  ...
  for (const edge of edges) {
    ...
    if (connectedNode) {
      ...
      candidatePath.smallCaveVisited = path.smallCaveVisited;
  ...
  while (incompletePaths.length > 0) {
    ...
    for (const path of candidatePaths) {
      ...
      if (lastNodeLabel === 'end') {
        ...
        if (nodeFound) {
          // Second time at this small cave
          if (path.smallCaveVisited === true) {
            // ignore this path
          }
          else {
            // Ok, but just for this small cave
            path.smallCaveVisited = true;
            incompletePaths.push(path);
          }
        }
        else {
          // First time at this small cave, keep going
          incompletePaths.push(path);
        }
      } else {
