...
async function runner() {
  ...
  for (let i = 0; i < sfNumbers.length; i++) {
    for (let j = 0; j < sfNumbers.length; j++) {
      if (i !== j) {
//        console.log(treeToString(sfNumbers[i]));
//        console.log(treeToString(sfNumbers[j]));
        const left = _.cloneDeep(sfNumbers[i]);
        const right = _.cloneDeep(sfNumbers[j]);
        const thisSum = sum(left, right);
        const sumMag = magnitude(thisSum);
//        console.log(sumMag);
        maxSum = Math.max(sumMag, maxSum);
      }
    }
  }
